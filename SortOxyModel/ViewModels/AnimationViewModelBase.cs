...
namespace AnimationsDemo
{
    ...
    public abstract class AnimationViewModelBase : IAnimationViewModel
    {
        ...
        public bool IsAnimationRuning { get; set; }

        public Task AnimateAsync(CancellationToken cancellationToken)
        {
            ...
            return this.AnimateAsync(cancellationToken, animationSettings);
        }

        public Task AnimateAsync2(CancellationToken cancellationToken, AnimationSettings animationSettings)
        {
            var easingFunction = (IEasingFunction)Activator.CreateInstance(this.SelectedEasingFunction);

            animationSettings.EasingFunction = easingFunction;
            animationSettings.Duration = TimeSpan.FromMilliseconds(this.AnimationDuration);
            animationSettings.FrameDuration = TimeSpan.FromMilliseconds(this.AnimationFrameDuration);
            animationSettings.Delay = TimeSpan.FromMilliseconds(this.AnimationDelay);
            animationSettings.HorizontalPercentage = this.HorizontalPercentage;
            animationSettings.VerticalPercentage = this.VerticalPercentage;
            
            return this.AnimateAsync(cancellationToken, animationSettings);
        }

        public abstract Task AnimateAsync(CancellationToken cancellationToken, AnimationSettings animationSettings);
    }
}